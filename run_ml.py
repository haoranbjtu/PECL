...
logging.basicConfig(filename='ml_train.log', level=logging.DEBUG, format=LOG_FORMAT, datefmt=DATE_FORMAT)
...
dropout_rate = 0.5  #
lr = 0.001  #
weight_decay = 0.015  # L2正则化系数
"""
etypes_lists = [[[0, 1], [0, 1, 0, 1], [0, None, 1]],  # item 顺序是根据关系多少确定  tools.py 
...
pathlen_lists = [[1, 2, 3, 4], [1, 2, 3, 4]]


with open(dir_path + '/conf/PECL.yaml', 'r', encoding='utf-8') as f:
    config = yaml.safe_load(f)

    # 将字典转为 Namespace 对象（方便用 args.xxx 访问）
    ag = argparse.Namespace(**config)
# 取正常完整数据集
outputdir = '/data/preprocessed/{}_dgl_all'.format(ag.dataset)  # './data/preprocessed/epinions_dgl_all'  # './data/preprocessed/ciao_dgl_all'
with open(dir_path + outputdir + '/homo_dataset.pkl', 'rb') as f:
...
def run_model_ML(feats_type, hidden_dim, num_heads, attn_vec_dim, rnn_type,
    ...
    ap = argparse.ArgumentParser(description='RWGNN testing for the ciao dataset')
    ...
    ap.add_argument('--rnn-type', default='RotatE1', help='Type of the aggregator. Default is RotatE1.')
    ...
    ap.add_argument('--batch-size', type=int, default=64, help='Batch size. Default is 8.')  # batch大 效果不好
    ...
    ap.add_argument('--num-paths-per-node', default=12, help='num_paths_per_node. Default is 12.')  # 20
    ...
    for _ in range(repeat):
        net = RWGNN_lp(
            [4, 4], 5, train_g.to(device), pathlen_lists, in_dims, hidden_dim, hidden_dim, num_heads, attn_vec_dim, rnn_type,
            dropout_rate)  # 边的类型两种 10 01  # [3, 4], 2,
        net.to(device)
        optimizer = torch.optim.Adam(net.parameters(), lr=lr, weight_decay=weight_decay)
        # net.load_state_dict(torch.load('checkpoint/checkpoint_{}.pt'.format(save_postfix)))  # 就这次 断点继续训练

        # training loop
        net.train()
        early_stopping = EarlyStopping(patience=patience, verbose=True,
                                       save_path=dir_path + '/checkpoint/checkpoint_{}.pt'.format(save_postfix))
        dur1 = []
        dur2 = []
        dur3 = []
        ...
        for epoch in range(num_epochs):
            ...
            for iteration in range(train_pos_idx_generator.num_iterations()):  # 一批次一批次的id输入模型计算 有分batch
                ...
                # if len(train_ratings[train_pos_idx_batch][:, 3].tolist()) < batch_size:  # 最后一batch总报错
                #     continue
                # train_pos_user_artist_batch = train_ratings[train_pos_idx_batch].tolist()
                ...
                [pos_embedding_user, pos_embedding_artist], _, rating_predictions, features = net(
                ...
                train_loss = contrastive_loss(pos_embedding_user, pos_embedding_artist)
                return train_loss
                ...
                # if iteration % 100 == 0:
                #     print(
                #         'Epoch {:05d} | Iteration {:05d} | Path_loss {:.4f} | Time1(s) {:.4f} | Time2(s) {:.4f} | Time3(s) {:.4f}'.format(
                #             epoch, iteration, train_loss, np.mean(dur1), np.mean(dur2),
                #             np.mean(dur3)))
                #     logging.info(
                #         'Epoch {:05d} | Iteration {:05d} | Path_loss {:.4f} | Time1(s) {:.4f} | Time2(s) {:.4f} | Time3(s) {:.4f}'.format(
                #             epoch, iteration, train_loss, np.mean(dur1), np.mean(dur2),
            ...
            #         rating_predictions = torch.bmm(embedding_item, embedding_user)  # 评分公式1 内积  # rating_label需要整成迭代器
            #         """
            #         val_loss = contrastive_loss(pos_embedding_user, pos_embedding_artist)
            #         # criterion = RMSELoss()
            ...
            #     print('Epoch {:05d} | RMSE {:.4f} | MAE {:.4f} | Time(s) {:.4f}'.format(
            #         epoch, torch.mean(val_loss), torch.mean(mae), t_end - t_start))
            #     logging.info('Epoch {:05d} | RMSE {:.4f} | MAE {:.4f} | Time(s) {:.4f}'.format(
            #         epoch, torch.mean(val_loss), torch.mean(mae), t_end - t_start))
            # # early stopping
    ...
    #     net.load_state_dict(torch.load(dir_path + '/checkpoint/checkpoint_{}.pt'.format(save_postfix)))
    #     net.eval()
    #     test_loss = []
    #     mae_list = []
    #     with torch.no_grad():
    ...
    #             criterion = RMSELoss()
    #             rmse, mae = criterion(rating_label, rating_predictions.view(-1))
    #             test_loss.append(torch.mean(rmse))  # rmse  记得正则化
    #             mae_list.append(torch.mean(mae))
    ...
    #         print('RMSE = {}, MAE = {}'.format(torch.sqrt(test_loss), mae.item()))
    #         logging.info('Rating Prediction Test')
    #         logging.info('AUC = {}, MAE = {}'.format(torch.sqrt(test_loss), mae.item()))
    #         test_loss_list.append(torch.sqrt(test_loss))
...
def test(feats_type, hidden_dim, num_heads, attn_vec_dim, rnn_type,
    ...
    net = RWGNN_lp(
        [4, 4], 5, train_g.to(device), etypes_lists, in_dims, hidden_dim, hidden_dim, num_heads, attn_vec_dim, rnn_type,
        dropout_rate)  # 边的类型两种 10 01  # [3, 4], 2,
    net.to(device)

    test_ratings = test_df[['iid', 'uid', 'rating', 'helpfulness']].to_numpy()
    test_loss_list = []
    test_mae_list = []
    test_idx_generator = index_generator(batch_size=batch_size*16, num_data=len(test_ratings), shuffle=False)
    net.load_state_dict(torch.load(dir_path + 'checkpoint/checkpoint_{}.pt'.format(save_postfix)))
    net.eval()
    test_loss = []
    mae_list = []
...
def contrastive_loss(user_emb, item_emb, temperature=0.5):
    """
    InfoNCE 对比损失
    user_emb: [B, D]
    item_emb: [B, D]
    ...
    labels = torch.arange(user_emb.size(0)).long().to(user_emb.device)  # 正样本对角线匹配
...
if __name__ == '__main__':
    ...
    ap.add_argument('--patience', type=int, default=5, help='Patience. Default is 5.')  # 早停
    ap.add_argument('--batch-size', type=int, default=2048, help='Batch size. Default is 8.')  # batch大 效果不好
    ap.add_argument('--samples', type=int, default=20, help='Number of neighbors sampled. Default is 100.')
    ap.add_argument('--repeat', type=int, default=1, help='Repeat the training and testing for N times. Default is 1.')
    ap.add_argument('--save-postfix', default='ML', help='Postfix for the saved model and result. Default is Ciao.')
    ap.add_argument('--num-paths-per-node', default=12, help='num_paths_per_node. Default is 12.')  # 20
    ap.add_argument('--restart-prob', default=0.2, help='num_paths_per_node. Default is 12.')  # 20
    ...
    run_model_ML(args.feats_type, args.hidden_dim, args.num_heads, args.attn_vec_dim, args.rnn_type, args.epoch,
                   args.patience, args.batch_size, args.samples, args.repeat, args.save_postfix)

    # test(args.feats_type, args.hidden_dim, args.num_heads, args.attn_vec_dim, args.rnn_type, args.epoch,
    #         args.patience, args.batch_size, args.samples, args.repeat, args.save_postfix)
__all__ = ["run_model_ML", "test"]
