...
def _series_to_tensor(series):
    if is_categorical(series):
        return torch.tensor(series.cat.codes.values.astype('int64'))
    else:  # numeric
        return torch.tensor(series.values, dtype=torch.float64)
...
class PandasGraphBuilder(object):
    ...
    def add_binary_relations(self, relation_table, source_key, destination_key, name):
        ...
        self.edges_per_relation[etype] = (torch.tensor(src.cat.codes.values, dtype=torch.int32),
                                          torch.tensor(dst.cat.codes.values, dtype=torch.int32))
