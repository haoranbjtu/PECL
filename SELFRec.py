...
class SELFRec(object):
    def __init__(self, config):
        ...
        # self.training_time = FileIO.load_time_set(config['training.set'], config['model']['type'])
        # self.test_time = FileIO.load_time_set(config['test.set'], config['model']['type'])
    ...
    def execute(self):
        ...
        # recommender = f"{self.config['model']['name']}(self.config,self.training_data,self.test_data,self.training_time,self.test_time,**self.kwargs)"##################################这里是来源
