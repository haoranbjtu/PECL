...
# multi-layer support
# 非mini-batch
# RWGNN上层  需要搞清楚具体作用 对应哪个过程  然后 改细节
class RWGNN_layer(nn.Module):  # 聚合多种metapath
    def __init__(self,
                 num_metapaths_list,  # 不需要聚合元路径，但需要对user和Item单独计算/ user 和 item在同一个嵌入空间 先这样
                 num_edge_type,
                 etypes_lists,
                 in_dim,
                 out_dim,
                 ...
                 rnn_type='gru',
                 attn_drop=0.5):
        super(RWGNN_layer, self).__init__()
        self.in_dim = in_dim
        ...
        for i in range(len(num_metapaths_list)):
            self.ctr_ntype_layers.append(Paths_att_aggre(num_metapaths_list[i],
                                                                  etypes_lists[i],
                                                                  in_dim,
                                                                  num_heads,
                                                                  attn_vec_dim,
...
class RWGNN_nc(nn.Module):  # 最外层 整体网络两层
    def __init__(self,
                 num_layers,
                 num_metapaths_list,
                 num_edge_type,
        ...
        for l in range(num_layers - 1):
            self.layers.append(RWGNN_layer(num_metapaths_list, num_edge_type, etypes_lists, hidden_dim, hidden_dim,
                                              num_heads, attn_vec_dim, rnn_type, attn_drop=dropout_rate))
        # output projection layer
        self.layers.append(RWGNN_layer(num_metapaths_list, num_edge_type, etypes_lists, hidden_dim, out_dim,
                                          num_heads, attn_vec_dim, rnn_type, attn_drop=dropout_rate))
    ...
    def forward(self, inputs, target_node_indices):
        ...
        # ntype-specific transformation
        transformed_features = torch.zeros(type_mask.shape[0], self.hidden_dim, device=features_list[0].device)  # zero?
        ...
        for l in range(self.num_layers - 1):
            ...
            h = F.elu(h)
        # output projection layer
        logits, h = self.layers[-1]((g_lists, h, type_mask, edge_metapath_indices_lists))  # 节点分类任务
