...
from run_ml import run_model_ML, test

class PECL(GraphRecommender):
    def __init__(self, conf, training_set, test_set, training_time, test_time):
        super(PECL, self).__init__(conf, training_set, test_set, training_time, test_time)
    # def __init__(self, conf, training_set, test_set):
        # super(PECL, self).__init__(conf, training_set, test_set)
    ...
    def path_loss(self, path_emb, initial_emb, user, item):
        # 1. 调用 run_model_ML，获得路径嵌入
        # 例如 RWGNN 模型生成的 path embedding 或预测结果
        feats_type = 0
        hidden_dim = 64
        num_heads = 8
        attn_vec_dim = 64
        rnn_type = 'RotatE1'
        num_epochs = 1  # 这里只跑一轮以避免过慢
        patience = 3
        batch_size = 256
        neighbor_samples = 20
        repeat = 1
        save_postfix = "path_loss_tmp"

        try:
            path_loss = run_model_ML(
                feats_type, hidden_dim, num_heads, attn_vec_dim,
                rnn_type, num_epochs, patience, batch_size,
                neighbor_samples, repeat, save_postfix
            )
        except Exception as e:
            print("⚠️ run_model_ML 调用失败:", e)
            return torch.tensor(0.0, device=path_emb.device)
        return path_loss
    ...
    def train(self):
        ...
        for epoch in range(self.maxEpoch):
            ...
            for n, batch in enumerate(next_batch_pairwise(self.data, self.batch_size)):
                ...
                rec_user_emb, rec_item_emb, emb_list, path_list, time_list  = model()
                ...
                ssl_path_loss = self.ssl_path_loss(context_emb,path_emb,user_idx,pos_idx)
                ...
                if epoch<20: #warm_up
                    # path_loss = self.path_loss(context_emb,path_emb,user_idx,pos_idx)
                    warm_up_loss = rec_loss + l2_reg_loss(self.reg, user_emb, pos_item_emb, neg_item_emb)/self.batch_size  + ssl_loss + ssl_path_loss + multi_loss
                    optimizer.zero_grad()
                    warm_up_loss.backward()
                    optimizer.step()
                    if n % 100 == 0 and n > 0:
                        print('training:', epoch + 1, 'batch', n, 'rec_loss:', rec_loss.item(), 'ssl_loss', ssl_loss.item())
                        # print('training:', epoch + 1, 'batch', n, 'rec_loss:', rec_loss.item(), 'ssl_loss', ssl_loss.item(), multi_loss.item())
                        # print('training:', epoch + 1, 'batch', n, 'rec_loss:', rec_loss.item(), 'path_loss', path_loss.item(), 'multi_loss', multi_loss.item())
                else:
                    ...
                    batch_loss = rec_loss + l2_reg_loss(self.reg, user_emb, pos_item_emb, neg_item_emb) / self.batch_size + ssl_loss + multi_loss + proto_loss + ssl_path_loss# + time_loss
                    ...
                    if n % 100 == 0 and n > 0:
                        # print('training:', epoch + 1, 'batch', n, 'rec_loss:', rec_loss.item(), 'ssl_loss', ssl_loss.item(), multi_loss.item())
                        print('training:', epoch + 1, 'batch', n, 'rec_loss:', rec_loss.item(), 'ssl_loss', ssl_loss.item(), 'proto_loss', proto_loss.item(), 'path_loss', ssl_path_loss.item())
                        # print('training:', epoch + 1, 'batch', n, 'rec_loss:', rec_loss.item(), 'ssl_loss', ssl_loss.item(), 'proto_loss', proto_loss.item(), 'multi_loss', multi_loss.item())
            ...
            with torch.no_grad():
                ...
                # self.user_emb, self.item_emb, _, path = model()# 
                self.user_emb, self.item_emb, _, path, time = model()# 
                # self.user_emb, self.item_emb, _, rate, time, path = model()# 
    ...
    def save(self):
        with torch.no_grad():
            ...
            self.best_user_emb, self.best_item_emb, _, path, time = self.model()
...
class LGCN_Encoder(nn.Module):
    ...
    def forward(self):
        ...
        for k in range(self.layers):
            ...
            # ego_embeddings4 = torch.sparse.mm(self.sparse_norm_adj, ego_embeddings)
        ...
        return user_all_embeddings, item_all_embeddings, all_embeddings, all_path_embeddings, all_time_embeddings
